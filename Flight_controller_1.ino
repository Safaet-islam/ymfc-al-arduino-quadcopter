...
void loop(){
  ...
  gyro_yaw_input = (gyro_yaw_input * 0.98) + ((gyro_yaw / 65.5) * 0.02);      //Gyro pid input is deg/sec.

  //--- begin yaw-deadband & stick-centering hack ---
  // 1) Don’t yaw unless stick is off-center by ≥20µs:
  if (abs(receiver_input_channel_4 - 1500) < 20) {
    pid_yaw_setpoint = 0;
  }
  // 2) Throw away small Z-noise (<3°/s):
  float rawZ = gyro_yaw / 65.5;
  if (abs(rawZ) < 3.0) rawZ = 0;
  // 3) Re-filter yaw with deadbanded raw:
  gyro_yaw_input = (gyro_yaw_input * 0.90) + (rawZ * 0.10);
  //--- end yaw-deadband & stick-centering hack ---
  ...
  Serial.print(" | Yaw: "); Serial.println(gyro_yaw);
...
void gyro_signalen(){
  ...
  gyro_yaw = -1* gyro_axis[eeprom_data[30] & 0b00000011];                       //Set gyro_yaw to the correct axis that was stored in the EEPROM.
...
void calculate_pid(){
  ...
  // clamp tiny yaw outputs to zero
  if (abs(pid_output_yaw) < 10) pid_output_yaw = 0;
